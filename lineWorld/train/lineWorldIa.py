...
class LineWorldIa:
    ...
    def __init__(self, numStates, modelPath):

        self.numStates = numStates
        self.modelPath = modelPath
        self.states = pd.DataFrame({"state": np.arange(numStates)})
        self.actions = pd.DataFrame({"action": [0, 1]})
        self.terminal = pd.DataFrame({"state": [0, numStates - 1]})
        ...
        for state in self.states["state"]:
            ...
            for action in self.actions["action"]:
                ...
                for nextState in self.states["state"]:
                    ...
                    for reward in [0, 1]:
                        self.initProb(state, action, nextState, reward, 0.0)

        self.probs = pd.DataFrame(self.probs).set_index(["state", "action", "nextState", "reward"])

        for i in range(1, self.numStates - 1):
            self.probs.loc[i, 0, i - 1, 0] = 1.0
            self.probs.loc[i, 1, i + 1, 0] = 1.0

        self.probs.loc[1, 0, 0, 1] = -1.0
        self.probs.loc[self.numStates - 2, 1, self.numStates - 1, 1] = 1.0
    ...
    @staticmethod
    def getBestActionWithValues(values, state):

        return 0 if values.loc[state - 1][0] > values.loc[state + 1][0] else 1

    @staticmethod
    def getBestActionWithPolicies(policies, state):
    ...
    def policyIteration(self, gamma, theta):
        ...
        while True:
            ...
            for state in self.states["state"]:
                ...
                action = self.getBestActionWithPolicies(policies, state)
                bestAction = 0
                bestActionScore = -999

                for act in self.actions["action"]:

                    total = 0.0

                    for nextState in self.states["state"]:
                        total += self.probs.loc[state, act, nextState, 0][0] * \
                                 (self.probs.loc[state, act, nextState, 1][0] +
                                  gamma * values.loc[nextState][0])
    ...
    def monteCarloExplorControl(self, nbEpisode=10000, maxStepPerEp=10, gamma=0.99):

        policies = self.getPolicies()
        probByAct = pd.DataFrame({"state": self.states["state"], 0: np.random.random(self.numStates),
                                  1: np.random.random(self.numStates)}).set_index("state")

        for state in self.states["state"]:
            if self.isTerminal(state):
                probByAct.loc[state] = 0.0
                policies.loc[state, 0] = 0.0
                policies.loc[state, 1] = 0.0
    ...
    @staticmethod
    def getActionWithPolicies(policies, state):
        return policies.loc[state].idxmax()[0]

    @staticmethod
    def getActionWithActProb(actProb, state):
